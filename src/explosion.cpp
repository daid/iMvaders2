...
: sp::ParticleEmitter(space_scene->getRoot())
{
    ...
    for(int n=0; n<10; n++)
    {
        ...
        parameters.velocity = sp::normalize(sp::Vector3f(sp::random(-1, 1), sp::random(-1, 1), 0.0)) * sp::random(1.5 * size, 2.5 * size) + sp::Vector3f(velocity.x * 0.7, velocity.y * 0.7, 0);
        parameters.acceleration = -parameters.velocity * 0.5f;
        parameters.start_size = size * 0.7;
        parameters.end_size = size * 0.3;
        parameters.start_color = sp::Vector3f(1, 0.5, 0.2);
        parameters.end_color = sp::Vector3f(0, 0, 0);
    ...
    for(int n=0; n<30; n++)
    {
        parameters.position = sp::normalize(sp::Vector3f(sp::random(-1, 1), sp::random(-1, 1), 0.0)) * sp::random(0.3 * size, 0.5 * size);
        ...
        parameters.start_color = sp::Vector3f(1, 0.2, 0.1);
        parameters.end_color = sp::Vector3f(0, 0, 0);
