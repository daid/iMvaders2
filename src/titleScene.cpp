...
TitleController::TitleController(sp::P<sp::Scene> scene)
: sp::SceneNode(scene->getRoot())
{
    sp::P<sp::CameraNode> camera = new sp::CameraNode(scene->getRoot());
    ...
    sp::P<sp::SceneNode> background = new sp::SceneNode(scene->getRoot());
    ...
    title = new sp::SceneNode(scene->getRoot());
    title->render_data = sp::SpriteManager::get("logo2");
    title->setPosition(sp::Vector2d(0, 15));

    sp::P<sp::SceneNode> text_scroll_root = new sp::SceneNode(scene->getRoot());
    ...
    graphics_layer->addRenderPass(new sp::BasicNodeRenderPass("window", scene));
}
